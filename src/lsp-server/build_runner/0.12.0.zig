...
fn extractBuildInformation(
    ...
    // adds -fincremental to args for us
    b.graph.incremental = true;

    var root_imports: std.ArrayListUnmanaged(BuildConfig.NamePathPair) = .{};
    var roots: std.ArrayListUnmanaged(BuildConfig.RootEntry) = .{};
    ...
    for (b.top_level_steps.values(), 0..) |tls, i| {
        ...
        for (tls.step.dependencies.items) |step| {
            ...
            for (step.dependencies.items) |dep_step| {
                ...
                var cli_named_modules = try CliNamedModules.init(gpa, &compile.root_module);
                ...
                const args = try getZigArgs(compile, false);
                const root_entry: BuildConfig.RootEntry = .{
                    .args = args,
                    .mods = try root_imports.toOwnedSlice(gpa),
                };
                try roots.append(gpa, root_entry);
