...
pub fn buildOutputType2(
            ...
            args_loop: while (args_iter.next()) |arg| {
                if (mem.startsWith(u8, arg, "@")) {
                        ...
                        mem.eql(u8, arg, "-needed_library"))
                    {
                        ...
                        cs.create_module.opts.lto = true;
                    } else if (mem.eql(u8, arg, "-fno-lto")) {
                        cs.create_module.opts.lto = false;
                    } else if (mem.eql(u8, arg, "-funwind-tables")) {
        ...
        .cc, .cpp => {
            ...
            while (it.has_next) {
                ...
                switch (it.zig_equivalent) {
                    ...
                    .lto => cs.create_module.opts.lto = true,
                    .no_lto => cs.create_module.opts.lto = false,
    ...
    implicit_root_mod: {
        ...
        if (cs.mod_opts.unwind_tables) |uwt| switch (uwt) {
            .none => {},
            .sync => if (cs.create_module.opts.any_unwind_tables == .none) {
                cs.create_module.opts.any_unwind_tables = .sync;
            },
            .@"async" => {
                cs.create_module.opts.any_unwind_tables = .@"async";
            },
        };
